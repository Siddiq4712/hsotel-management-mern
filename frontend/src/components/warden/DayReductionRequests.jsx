      ...
      <div className="p-8 bg-slate-50 min-h-screen space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <Title level={2} style={{ margin: 0 }}>Day Reduction Review</Title>
              <Text type="secondary">Review and track mess bill reduction requests handled by the Office</Text>
            </div>
          </div>
          <Button icon={<RefreshCw size={16}/>} onClick={fetchRequests} className="rounded-xl h-11 px-6 font-bold shadow-sm">
            Refresh List
          </Button>
        </div>
            ...
            <Row gutter={[24, 24]}>
              <Col xs={24} md={8}>
                <Card className="border-none shadow-sm rounded-2xl">
                  <Statistic 
                    title={<span className="text-[10px] uppercase font-bold text-slate-400 tracking-widest">Office Approved</span>} 
              ...
              <Col xs={24} md={8}>
                <Card className="border-none shadow-sm rounded-2xl">
                  <Statistic 
                    title={<span className="text-[10px] uppercase font-bold text-slate-400 tracking-widest">Total Reductions</span>} 
              ...
              <Col xs={24} md={8}>
                <Card className="border-none shadow-sm rounded-2xl">
                  <Statistic 
                    title={<span className="text-[10px] uppercase font-bold text-slate-400 tracking-widest">Verified by Warden</span>} 
              ...
              <div className="flex flex-wrap gap-4 items-center">
                ...
                <div className="flex items-center gap-3 bg-slate-50 p-1 px-3 rounded-xl border border-slate-100">
                  ...
                  >
                    ...
                    <Select.Option value="approved_by_admin">Approved by Office</Select.Option>
                    <Select.Option value="pending_admin">Pending Office Approval</Select.Option>
              ...
              {filteredData.length > 0 ? (
                ...
                <div className="py-24">
                  <Empty 
                    ...
                    description={<Text className="text-slate-400 block">No reduction requests found in the register.</Text>}
                  />
                </div>
              )}
            </Card>
        ...
        <Modal
          title={<div className="flex items-center gap-2 text-blue-600"><ClipboardList size={20}/> Reduction Request Details</div>}
          open={isModalVisible}
          onCancel={() => setIsModalVisible(false)}
          footer={<Button type="primary" onClick={() => setIsModalVisible(false)} className="rounded-xl h-11 px-8 font-bold">Close</Button>}
          width={600}
            ...
            <div className="mt-6 space-y-6">
              ...
              <div className="grid grid-cols-1 gap-4">
                <div className="p-4 rounded-2xl bg-blue-50/50 border border-blue-100">
                  <Text strong className="text-[11px] uppercase text-blue-400 block mb-2 tracking-widest">Office Remarks</Text>
                  <Text className="text-sm">{selectedRequest.admin_remarks || 'No remarks recorded.'}</Text>
                </div>
                <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200">
                  <Text strong className="text-[11px] uppercase text-slate-400 block mb-2 tracking-widest">Warden Comments</Text>
                  <Text className="text-sm italic text-slate-500">{selectedRequest.warden_remarks || 'No prior warden comments.'}</Text>
                </div>
              </div>
              ...
              <div className="p-4 bg-amber-50 rounded-2xl border border-amber-100 flex gap-3">
                ...
                <Text className="text-[11px] text-amber-700 leading-tight">
                  Note: Day reductions are processed by the Office. Warden access is for reviewing and verifying the records.
                </Text>
              </div>
            </div>
          )}
