...
const ManageRebates = () => {
  ...
  const columns = [
    ...
    {
      title: 'Detailed Monthly Split-up',
      key: 'split',
      render: (_, r) => (
    ...
    },
    {
      ...
      render: (_, r) => (
        <div className="flex flex-col items-center justify-center bg-emerald-50 border border-emerald-100 p-2 rounded-2xl min-w-[100px]">
           <Text className="text-[9px] text-emerald-600 font-black uppercase tracking-tighter">System Total</Text>
    ...
    },
    {
      ...
      render: (_, r) => r.status === 'pending' && (
        <Space>
          <Button type="primary" onClick={() => { setSelectedRebate(r); setIsApproveModalOpen(true); }} className="rounded-lg bg-blue-600 font-bold border-none shadow-md">
            Review
          </Button>
          <Popconfirm title="Reject?" onConfirm={() => wardenAPI.updateRebateStatus(r.id, { status: 'rejected' }).then(fetchRebates)}>
      ...
      <div className="p-8 bg-slate-50 min-h-screen space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-blue-600 rounded-2xl shadow-lg shadow-blue-100 text-white"><Percent size={24} /></div>
            <div>
              <Title level={2} style={{ margin: 0 }}>Rebate Approval Board</Title>
              <Text type="secondary">Automated credit calculations based on month-wise audited daily rates</Text>
            </div>
          </div>
        ...
        <Card className="border-none shadow-sm rounded-[32px] overflow-hidden" bodyStyle={{ padding: 0 }}>
          <div className="p-6 bg-white border-b border-slate-50 flex justify-between items-center">
            <Text strong className="text-slate-400 text-[10px] uppercase tracking-widest">Active Applications</Text>
            <div className="flex gap-2 bg-slate-100 p-1 rounded-xl">
              <Select value={statusFilter} onChange={setStatusFilter} bordered={false} className="w-32 font-bold text-slate-600">
          ...
          <Table columns={columns} dataSource={rebates} loading={loading} rowKey="id" />
        </Card>

        {/* Breakdown Modal */}
        <Modal
          title={<div className="flex items-center gap-2"><Calculator size={20} className="text-blue-600"/> Audit Verification</div>}
          open={isApproveModalOpen}
          onCancel={() => setIsApproveModalOpen(false)}
          onOk={handleApprove}
          okText="Confirm & Approve Credit"
          centered
            ...
            <div className="mt-4 space-y-6">
              ...
              <div className="bg-slate-900 p-8 rounded-[32px] text-white relative overflow-hidden shadow-2xl">
                 <div className="relative z-10">
                   <Text className="text-slate-400 uppercase text-[10px] font-black tracking-widest mb-2 block">Total Rebate Payable</Text>
                   <div className="flex items-baseline gap-2">
                      <span className="text-2xl text-emerald-400 font-bold">₹</span>
                      <span className="text-6xl font-black">{selectedRebate.calculationDetail?.total}</span>
                   </div>
                   ...
                   <div className="flex gap-6">
                      <div>
                        ...
                        <Text className="text-white text-xs font-bold uppercase">{selectedRebate.RebateStudent?.username}</Text>
                      </div>
                      <div>
                        <Text className="text-slate-500 text-[9px] font-bold block uppercase">Total Days</Text>
                        <Text className="text-white text-xs font-bold uppercase">{selectedRebate.calculationDetail?.totalDays} Days</Text>
                      </div>
                   </div>
                 </div>
                 <div className="absolute -bottom-10 -right-10 opacity-10"><Landmark size={180}/></div>
              </div>

              {/* Multi-Month Split Section */}
              <div className="space-y-3">
                <Text strong className="text-slate-500 uppercase text-[10px] tracking-widest flex items-center gap-2">
                    <Calendar size={14}/> Month-wise Calculation Split
                </Text>
                {selectedRebate.calculationDetail?.monthlySplit.map((item, idx) => (
                  <div key={idx} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 transition-all hover:border-blue-200 shadow-sm">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <Text className="block text-sm font-black text-slate-800 uppercase">{item.label}</Text>
                        {!item.isAudited && <Badge status="warning" text={<span className="text-[8px] font-bold text-amber-600 uppercase">Est. Rate</span>} />}
                      </div>
                      <Text type="secondary" className="text-xs font-medium">
                        {item.daysCount} Days Absence × ₹{item.ratePerDay} <span className="text-[10px] opacity-50">(Daily Rate)</span>
                      </Text>
                    </div>
                    <div className="text-right">
                      <Text strong className="block text-blue-600 text-lg">₹{item.subTotal.toFixed(2)}</Text>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="p-4 bg-blue-50 rounded-2xl border border-blue-100 flex gap-3">
                ...
                <Text className="text-blue-700 text-[11px] leading-relaxed">
                  These rates are automatically fetched from the <strong>Daily Rate Ledger</strong>. 
                  Approving this will deduct this total amount from the student's next generated mess bill.
                </Text>
              </div>
            </div>
          )}
...
const Landmark = ({ size }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="3" y1="21" x2="21" y2="21"></line><line x1="9" y1="8" x2="9" y2="17"></line><line x1="15" y1="8" x2="15" y2="17"></line><path d="M4 11V3a1 1 0 0 1 1-1h14a1 1 0 0 1 1 1v8"></path></svg>;
