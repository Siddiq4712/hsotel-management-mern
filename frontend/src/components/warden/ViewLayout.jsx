  ...
  if (loading) return (
    <div className="flex h-screen items-center justify-center bg-white">
      <div className="flex flex-col items-center gap-3">
        ...
        <span className="text-[10px] font-black uppercase tracking-widest text-blue-900">Rendering Blueprint...</span>
      </div>
    </div>
  );
  ...
  if (!layoutConfig) return (
    <div className="p-12 text-center">
      <div className="inline-block p-6 border-2 border-dashed border-slate-200 rounded-lg">
        ...
        <p className="text-sm font-bold text-slate-400 uppercase tracking-tight">No layout has been committed yet.</p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#fcfdff] p-8 font-serif">
      <header className="max-w-[1400px] mx-auto flex justify-between items-center border-b-2 border-blue-900 pb-6 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black uppercase tracking-tighter text-blue-900 italic">Hostel Blueprint View</h1>
            <p className="text-[9px] font-bold text-blue-400 uppercase tracking-widest">Read-Only Architectural Map | {layoutConfig.building_type}-Shape</p>
          </div>
        </div>
        <div className="flex gap-4">
            <div className="px-4 py-2 bg-white border border-blue-100 rounded flex items-center gap-2">
                ...
                <span className="text-[9px] font-bold uppercase text-slate-500">Main Entrance: Side {layoutConfig.entrance_side.toUpperCase()}</span>
            </div>
        </div>
      </header>
      ...
      <footer className="max-w-[1400px] mx-auto mt-8 grid grid-cols-4 gap-4">
          {Object.entries(ROOM_TYPES).map(([key, cfg]) => (
              <div key={key} className="bg-white p-3 border border-blue-50 flex items-center gap-3 shadow-sm">
                  ...
                  <span className="text-[10px] font-bold uppercase text-slate-600">{cfg.name}</span>
              </div>
          ))}
      </footer>
    </div>
...
function GridCell({ type, data, entranceSide }) {
  ...
  if (type.startsWith("WALK_")) {
    ...
    return (
      <div className={`w-full h-full relative border-[0.5px] border-blue-100 flex items-center justify-center transition-colors ${isEntrance ? 'bg-orange-50/50' : ''}`}>
  ...
  return (
    <div className="w-full h-full p-1">
      <div className={`w-full h-full border-[1.5px] border-blue-900 flex flex-col ${data ? 'bg-white shadow-sm' : 'bg-transparent border-dashed border-blue-100'}`}>
        {data && Icon ? (
          <>
            <div className="px-1 border-b border-blue-50 text-[8px] font-serif font-bold text-blue-900 py-0.5">{data.room_number}</div>
            <div className="flex-1 flex items-center justify-center opacity-30"><Icon size={20} className="text-blue-900" /></div>
